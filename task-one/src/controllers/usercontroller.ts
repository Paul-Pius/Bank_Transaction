...
const generateAccountNum = async () => {
  const accountNumber = (Math.random() * 10 ** 10).toFixed(0);
...
const getTransactions = asyncHandler(async (req: Request, res: Response) => {
  ...
  const limitedDebitTransaction = allDebitTransaction.slice(
    startIndex,
    endIndex
  );
  const limitedCreditTransaction = allCreditTransaction.slice(
    startIndex,
    endIndex
  );
...
const getReceiverAccount = asyncHandler(async (req: Request, res: Response) => {
  const userId = req.cookies.Uid;
  const user = await Viewable.find({ userId: userId });
...
const performTransactions = asyncHandler(
